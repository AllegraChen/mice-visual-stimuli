...
#TODO# PLOT FOR FIRING RATES OF DIFFERENT MICE AND SESSIONS.
Analysis: There seems to be a pattern of similarity for the same mouse.
...
#TODO# Add in reference section usage of code from description.Rmd file provided
...
### Choice of Variables and Inclusion of Effects
The mouse name, session ID and the experiment date are not included as random effects in purpose. Since the data was collected from only 2 mice, there are simply too few mouse samples to consider the random effects from different mice. In addition to the similar reason as that of the exclusion of mouse name, <the EDA plot #TODO# FILL IN WHICH FIGURE> does not illustrate a strong evidence that neurons perform randomly in each session or different dates. 



?QUESTION? SIMPLE RANDOMIZED EXPERIMENT? ANYTHING TO SAY?
...
?QUESTION? Causal effect, potential outcome framework for hypo testing?
            Is date a random effect? What to do with date?
